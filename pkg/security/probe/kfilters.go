...
func (f *FilterPolicy) Bytes() ([]byte, error) {
	...
	if err := binary.Write(buffer, byteOrder, f.Mode); err != nil {
		return nil, err
	}
	if err := binary.Write(buffer, byteOrder, f.Flags); err != nil {
		return nil, err
	}
	rep := make([]byte, 2)
...
func StringToKey(str string, size int) ([]byte, error) {
	...
	buffer := new(bytes.Buffer)
	if err := binary.Write(buffer, byteOrder, []byte(str)[0:size]); err != nil {
		return nil, err
	}
	rep := make([]byte, size)
	copy(rep, buffer.Bytes())
...
func (k CommTableKey) Bytes() ([]byte, error) {
	return StringToKey(string(k), 16)
}
