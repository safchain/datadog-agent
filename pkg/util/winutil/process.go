...
func ReadProcessMemory(h windows.Handle, from uintptr, to uintptr, count uint32) (bytesread uint64, err error) {
	var bytes uint64
...
type _peb struct {
	...
	ProcessParameters uintptr // pointer to _rtl_user_process_parameters
	// lots more stuff
}
...
func getCommandLineForProcess64(h windows.Handle) (cmdline string, err error) {
	...
	var pparams _rtl_user_process_parameters
...
func GetCommandLineForPid(pid uint32) (cmdline string, err error) {
	...
	if err != nil {
		err = fmt.Errorf("Failed to open process %v\n", err)
		return
	}
	defer windows.CloseHandle(h)
	cmdline, err = GetCommandLineForProcess(h)
	if err != nil {
		err = fmt.Errorf("Failed to get command line %v\n", err)
		return
	}
